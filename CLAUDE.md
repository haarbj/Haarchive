...
**Real light/dark toggle, defaulting to the visitor's system preference, falling back to light.** This reverses an earlier, more restrictive iteration of this site (dark-only, then a route-scoped light exception for the homepage/articles) — both are now superseded. `src/lib/theme.ts` is the single source of truth: a small `useSyncExternalStore`-based store (no external theme library — matches this project's dependency-minimalism, see §10), exposing a `useTheme()` hook (`{ theme, setTheme }`) that any component can read reactively. `src/components/theme-toggle.tsx` is the one user-facing control (Sun/Moon icon button, shows the *current* theme rather than the click target), mounted in `site-header.tsx`'s icon cluster on both desktop and the mobile menu.

`src/app/layout.tsx` always server-renders the light class set (no `dark` in the SSR'd `<html>` className — the server can't know a per-visitor preference). A pre-hydration inline `<script>` decides the real first-paint theme before React ever runs: a saved `localStorage` choice (key `"haarchive-theme"`) wins if one exists; otherwise `window.matchMedia("(prefers-color-scheme: dark)")` decides; otherwise light. `suppressHydrationWarning` on `<html>` is still there for the same reason as before (the inline script may add `dark` before hydration, a legitimate, expected mismatch). Once a visitor clicks the toggle, their explicit choice is saved and wins over system preference from then on, for every route — this is now a genuinely global, user-controlled setting, not a route-conditional one, so there's no more per-route/per-navigation logic anywhere in the theme system.

Components still carry both light-mode and `dark:` Tailwind classes throughout the codebase (e.g. `text-zinc-900 dark:text-white`) — that's what makes the toggle work at all; this was already true before the toggle existed (a deliberate, long-standing convention, not something newly introduced). **Do not remove the light-mode classes.** One deliberate, separately-motivated exception: `Diagram` (`src/components/ui/diagram.tsx`, scientific/data figures) keeps its `bg-zinc-950` box permanently dark regardless of the site theme — see §3's "paper = writing, dark box = science" visual language. Don't make it theme-aware without a real reason to revisit that.
...
- Container widths (see §7 below): `--container-content`/`--container-wide` (1160px, unified), `--container-dashboard` (900px), `--container-narrow` (680px), `--container-auth` (400px), `--container-article-prose` (72ch).
...
**Migrated from an 8-category taxonomy to a four-domain one** (see `sections.ts`'s own comment above the `categories` array). The four domains — Physiology, Psychology, Philosophy, Practice — are the site's actual intellectual architecture now, matching the brand's own "physiology, psychology, philosophy, practice" language, not just homepage framing; the Library (slug `archive`) and Tools are two supporting, non-domain areas alongside them. Six `category` values total, in declared order (section counts as of this writing — check the file directly, since these change):

| Category slug | Title | Sections |
|---|---|---|
| `physiology` | Physiology | `exercise-physiology`, `the-aerobic-base`, `research-library`, `data-and-analytics`, `what-a-race-result-can-tell-you`, `nutrition-and-fueling`, `recovery`, `training-across-the-menstrual-cycle`, `pregnancy-and-postpartum-running`, `training-through-the-decades` |
| `psychology` | Psychology | `sports-psychology`, `goal-setting`, `self-talk`, `daily-practice`, `performing-under-pressure`, `for-coaches` |
| `philosophy` | Philosophy | `training-philosophy`, `the-philosophy-of-running`, `mostly-easy-genuinely-hard` |
| `practice` | Practice | `how-to-start-running`, `coaching-library`, `athlete-library`, `marathon-training`, `workout-library`, `trail-and-ultra-training`, `strength-training`, `5k-training`, `training-plans` |
| `archive` | Library | `articles`, `resources`, `contact` |
| `tools` | Tools | ten calculator sections plus `choosing-a-pace-calculator` (an essay, not a calculator — see §7), see §7 |

**The Library category's slug is `archive`, not `library`** — a deliberate slug/title mismatch, found and fixed mid-migration: `/library` was already a live, unrelated authenticated route (`src/app/(app)/(protected)/library/page.tsx`, the signed-in personal notes/saved-topics/history dashboard), and Next.js always resolves that static route ahead of the dynamic `[slug]` page for an exact path match — using `library` as this category's slug would have made its own canonical URL permanently unreachable. The title stays "Library" everywhere it's displayed (nav, footer, archive index); only the URL segment differs. This is the same tolerated slug/title-mismatch pattern the site used for `mind-and-recovery`, reused here for a different reason (a real route collision, not link stability).

The old 8 category slugs (`getting-started`, `foundations`, `the-science`, `coaching-and-training`, `recovery-and-fueling`, `mind-and-recovery`, `writing-and-resources`) 301-redirect to their new domain via `next.config.ts`'s `redirects()` — individual section/article/tool URLs underneath a category were never renamed, only which category each section belongs to changed. `writing-and-resources` redirects to `/archive`.

Note: this supersedes a prior documented decision that `mind-and-recovery`'s slug should stay stale for link stability. That reasoning has been **deliberately reversed** as part of this migration — link stability into that category is now preserved through the redirect above instead of slug preservation. Don't be confused if an older commit/doc references the old rationale; the redirect is the current mechanism.

**New content goes where a reader would naturally look for it.** Before adding a new top-level category, check whether the content actually belongs inside an existing section or category first — a new top-level category should be rare now that the taxonomy is organized around four broad, intentionally spacious domains rather than many narrow ones. The Lydiard-folder synthesis work (`docs/lydiard-synthesis/`) is a recent, real example of this reasoning in practice: new content went into existing thin sections and a new *subsection* of an existing page (`workout-library`'s technique/footwear cluster), not a new top-level page.

`coaching-library` and `athlete-library` are members of `practice` but have **no `content` array** in `sections.ts` — they're rendered by their own data-driven directory components (see §7).
...
|---|---|---|
| `content` | 1160px | Category landings, every section page (tool, article, list, placeholder), the Articles index |
| `wide` | 1160px | The homepage specifically — same number as `content` today, kept as its own variant because it still means something distinct (the homepage's own architectural width, not "reading pages" generally) |
...
`content` and `wide` were unified at 1160px (previously `content` was 1080px, homepage-only on `wide`) so header and content agree on one width across every reading/browsing page, not just the homepage — see `globals.css`'s comment on `--container-chrome`/`--container-content`/`--container-wide` for the reasoning. Authenticated app pages (`dashboard`/`narrow`/`auth`) are deliberately unaffected — they're denser tables and single-column forms, not a reading experience, with their own real width reasons.
...
- **Mobile is not a shrunk desktop layout for navigation.** `src/components/site-header.tsx` renders a genuinely different structure on mobile once the menu is open: the header itself becomes `fixed inset-0` (a true full-viewport panel, not an inline accordion that pushes page content down while leaving it visible/scrollable underneath), with the menu list as the only `flex-1 overflow-y-auto` region so only it scrolls, not the whole page. Mobile search is a toggled icon next to the hamburger, not a permanently-visible row — it was originally persistent specifically so it was never buried behind the hamburger, but that traded away vertical space on every mobile page whether or not a visitor wanted search, which became the actual complaint; toggling keeps it one tap away without the standing cost. The mobile "Learn" submenu shows **category names only**, not every section under every category — a full sub-list flattened the entire ~40-link site map into one screen, which was the exact problem worth avoiding; each category's own landing page is the real drill-down step and doesn't need to be reproduced in the nav.
- **Desktop nav is 6 independent top-level category items, each its own hover dropdown of that category's real sections** (`site-header.tsx`, `openCategorySlug` state) — not a shared "Learn" umbrella. This reverses a prior documented decision: with the old 8-category taxonomy, individual top-level links were deliberately rejected as competing too much with the header's logo/search/account-icon space, so every category (except Tools, which got its own plain link) lived inside one shared "Learn" trigger's two-pane hover flyout (left pane: category names; right pane: swapped to whichever category was hovered). The four-domain IA migration (§6) brought that down to 6 categories, which is what made per-category top-level items viable again — each is a real `<Link>` to that category's own page (not a non-navigating toggle button the way "Learn" was), and hovering/focusing it opens its own dropdown directly beneath it, chevron rotating 180°, via the same 150ms hover-intent close timer the old flyout used (generalized to guard against a stale close from one trigger clobbering a different trigger's dropdown that opened in the meantime — see the `scheduleClose` comment in `site-header.tsx`). A category with more than 4 sections (Psychology, Practice, Physiology, Tools) renders its dropdown as a 2-column grid instead of one tall single column; Philosophy and Library stay single-column. Dropdown rows are text-only, no icons — none of the 5 non-Tools categories' sections have an icon assignment anywhere in the codebase (only Tools' calculators do, via `TOOL_VISUALS`), and this was a deliberate scope decision, not an oversight. Mobile is untouched by this change — see the mobile "Learn" bullet above, which still shows category names only via the same `learnCategories` (non-Tools) filter, kept in `site-header.tsx` specifically for mobile's use now that desktop no longer needs it.
...
- **Contrast:** both themes are real now (see §2) — `--heading`/`--body-text`/`--muted` and every other token have separate `:root`/`:root.dark` values in `globals.css`, and most components read them (or the equivalent literal `text-zinc-900 dark:text-white`-style pairs) rather than assuming one background always renders.
...
- A real, user-facing light/dark toggle (§2 — `src/lib/theme.ts`, `theme-toggle.tsx`), defaulting to system preference and falling back to light, superseding two earlier, more restrictive iterations of this site (dark-only, then a route-scoped light exception). `Diagram`'s permanently-dark figure box is the one deliberate exception that does *not* follow the toggle — see §2/§3.
- The four-domain category taxonomy (§6) — `physiology`/`psychology`/`philosophy`/`practice` plus `archive` (Library)/`tools`, with the old 8 category slugs 301-redirecting to their new domain. This superseded a prior decision to keep `mind-and-recovery`'s slug stale for link stability; that stability is now handled by the redirect instead (see §6's note).
