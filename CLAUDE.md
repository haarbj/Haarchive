...
- **Desktop's "Learn" dropdown is a two-pane hover flyout, not a flat list of every category's every section.** The left pane is just the 7 category names; hovering or focusing one swaps the right pane to that category's own sections (`activeCategorySlug` state in `site-header.tsx`, falling back to the category matching the current page, falling back to the first category, so the pane is never blank on open). This replaced an earlier single-panel version that had to fit every category's combined section list at once and needed repeated width/column/height rebalancing as content grew — the two-pane version scales to more categories (left pane just gets one more row) and more sections-per-category (right pane shows one category at a time) without that problem recurring. Categories deliberately did **not** move to individual top-level header links — the existing `TOOLS_SLUG`/`learnCategories` comment's scalability reasoning (a new category is "one more block in the menu below, never a new top-level nav item") still holds, and 8 top-level items would compete with the header's own logo/search/account-icon space in a way a single "Learn" trigger doesn't.
