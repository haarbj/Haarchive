...
import { addRaceResult, deleteRaceResult } from "./actions";
...
function DeleteButton({ raceResultId }: { raceResultId: string }) {
  const [isPending, startTransition] = useTransition();
  return (
    <button
      type="button"
      disabled={isPending}
      onClick={() => startTransition(() => deleteRaceResult(raceResultId))}
    ...
    <div className="space-y-5">
      {results.length > 0 && (
        <div className="space-y-2">
          {results.map((result) => (
            <Card key={result.id} padding="sm" shadow={false} className="flex items-center justify-between gap-3 text-sm">
              <div>
              ...
              <DeleteButton raceResultId={result.id} />
            </Card>
          ))}
        </div>
      )}
